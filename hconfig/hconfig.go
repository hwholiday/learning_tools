...
type HConfig interface {
	Load() error
	GetPath(key string) (HVal, error)
	WatchPaths(event WatchEvent) error
	Close() error
}
...
func (c *config) GetPath(key string) (HVal, error) {
...
func (c *config) WatchPaths(event WatchEvent) error {
	var err error
