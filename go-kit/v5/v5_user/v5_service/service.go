...
func (s baseServer) TestAdd(ctx context.Context, in Add) AddAck {
	...
	s.logger.Debug(fmt.Sprint(ctx.Value(ContextReqUUid)), zap.Any("调用 v5_service Service", "TestAdd 处理请求"), zap.Any("请求用户", fmt.Sprint(ctx.Value("name"))))
	ack := AddAck{Res: in.A + in.B}
	s.logger.Debug(fmt.Sprint(ctx.Value(ContextReqUUid)), zap.Any("调用 v5_service Service", "TestAdd 处理请求"), zap.Any("处理返回值", ack))
	return ack
}
